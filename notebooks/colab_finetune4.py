"""Colab round 4: bge-large + hard negatives + SYNTHETIC queries (mentor idea).

Paste into a Colab cell after `kagglehub.login()` (T4 GPU runtime). ~25 min.

Identical recipe to colab_finetune2.py (holdout 0.9225) plus synthetic
(query, doc) pairs generated by scripts/synth.py: every document title is
parsed back into its topic phrase and re-wrapped in the train query template
families. Rationale: test topics are disjoint from train topics (e.g. test
asks about iron/magnesium/sulphur deficiency, train never does), so the only
way the encoder meets those topic phrases during fine-tuning is synthetically.

Prints the honest topic-grouped holdout first — same split and seed as round
2, so compare directly against 0.9225 — then exports ft_embs4.npz. Drop it in
...
ex_tr = real_triplets(tr) + synth_triplets(holdout_docs)
print(f"holdout run: {len(real_triplets(tr))} real + "
      f"{len(ex_tr) - len(real_triplets(tr))} synthetic triplets "
      f"({len(holdout_docs)} docs quarantined)")

model = finetune(ex_tr, seed=0)
s_ft, _, _ = score(model, va["query"].tolist())
print(f"holdout nDCG@5 with synthetic data = "
      f"{ndcg5(s_ft, va.query_id.tolist(), rels):.4f}")
print("(round 2, same recipe minus synthetic: 0.9225)")
del model
torch.cuda.empty_cache()

# --- 2. full fine-tune, 2 seeds, averaged embeddings -------------------------
ex_full = real_triplets(train_q) + synth_triplets(set())
q_tr_list, q_te_list, d_list = [], [], []
for seed in (0, 1):
    m = finetune(ex_full, seed=seed)
...
np.savez("ft_embs4.npz", qtrain=avg_norm(q_tr_list),
         qtest=avg_norm(q_te_list), docs=avg_norm(d_list))
print("saved ft_embs4.npz")

from google.colab import files
files.download("ft_embs4.npz")
